...
def detect_outliers(data: np.ndarray, threshold_median: float = 0.5, threshold_gradient: float = 0.5, kernel_size: int = 9,
                    use_iqr: bool = True, use_gradient: bool = True) -> np.ndarray:
    """
    Detect local outliers in a 2D depth map using IQR and/or gradient deviation.
    ...
    if use_gradient and sobel is not None:
        ...
        gy = sobel(arr, axis=1)
        grad = np.hypot(gx, gy)
        medg = median_filter(grad, size=(kernel_size, kernel_size))
        diffg = np.abs(grad - medg)
        q3 = np.percentile(diffg[~np.isnan(diffg)], 75)
        mask |= diffg > (threshold_gradient * max(q3, 1e-9))
    return mask


def remove_outliers(data: np.ndarray, threshold_median: float = 0.5, threshold_gradient: float = 0.5, kernel_size: int = 9,
                    replace_with_nan: bool = False, use_iqr: bool = True, use_gradient: bool = True,
                    crop_z_max: Optional[float] = None) -> np.ndarray:
    """Replace detected outliers with local median or NaN."""
    arr = data.copy().astype(float)
    mask = detect_outliers(arr, threshold_median, threshold_gradient, kernel_size, use_iqr, use_gradient)
    if replace_with_nan:
        arr[mask] = np.nan
    else:
